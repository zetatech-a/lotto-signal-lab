...
수집기는 공식 결과 페이지에서 최신 회차를 읽고, 로컬 DB에 없는 회차만 추가합니다.
주의: 동행복권은 안정된 공개 개발자 API를 약속한 서비스가 아닙니다.
`collector.py`의 JSON 호출은 공식 사이트의 기존 호환 경로를 격리해 둔 것이며, 현재
사이트의 회차별 데이터 계약으로 검증된 것은 아닙니다.
